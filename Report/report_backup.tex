...
% ------------------------------------------------------------------------------
\begin{abstract}
This report presents the design and optimization of a GPU-accelerated Breadth-First Search (BFS) solver capable of traversing the Friendster social network (65.6M nodes, 3.6B edges) in \textbf{831 milliseconds}---a \textbf{26x speedup} over sequential CPU and 14.5x improvement over our initial GPU version. The key innovations are: (1) \textbf{Direct Queue Emission} using warp-aggregated atomics to eliminate O(N) distance scans; (2) \textbf{Hybrid Direction-Optimization} switching between Top-Down and Bottom-Up traversal; (3) \textbf{Delta-Varint Compression} reducing memory footprint by 37\%; and (4) \textbf{Zero-Copy Streaming} enabling processing of graphs larger than GPU VRAM. We also implement the Afforest algorithm for connected components. All code is publicly available.
\end{abstract}
...
V3 Baseline & Shared Memory Kernels & 12,009 & 1.8x \\
V4.1 Hybrid & Direction-Optimization & 1,200 & 18.1x \\
V4.3 Compressed & Zero-Copy + Varint & 996 & 21.8x \\
\textbf{V5.3 Final} & \textbf{Direct Queue Emission} & \textbf{831} & \textbf{26.1x} \\
\bottomrule
\end{tabular}
