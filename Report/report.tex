...
\subsection{VRAM-Resident Graph Analysis}
To quantify the algorithmic speedup without the PCIe bottleneck, we benchmarked a synthetic random graph (20M nodes, 200M edges, 2.0GB total) that fits entirely in VRAM.

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Version} & \textbf{Time (ms)} & \textbf{Speedup} & \textbf{Observation} \\
\midrule
Sequential CPU & 5,278 & 1.0x & Baseline \\
V2 Chunked & 475 & 11.1x & High serialization overhead \\
V3.3 Compressed & 381 & 13.8x & Decompression overhead dominates \\
V1 Dynamic & 279 & 18.9x & Low overhead, massive parallelism \\
\textbf{V3.1 Shared} & \textbf{191} & \textbf{27.6x} & \textbf{Optimal for small/uniform graphs} \\
\bottomrule
\end{tabular}
\caption{BFS Performance on VRAM-Resident Graph (20M nodes)}
\end{table}

The results lead to a critical insight: \textbf{Complexity implies Overhead}. On this smaller, uniform-degree graph (Avg=10) that fits in memory:
\begin{itemize}
    \item \textbf{V1 (Dynamic)} outperforms V3.3 because the simple global atomic queue has minimal instruction overhead compared to the complex hybrid logic and Varint decompression of V3.3.
    \item \textbf{V3.1 (Shared)} is the fastest (191ms) because it combines the low overhead of V1 with Warp-Aggregation, reducing the number of global atomic adds by $\approx$30x compared to V1.
    \item \textbf{V3.3 (Compressed)} is slower here (381ms) because bandwidth is \textit{not} the bottleneck (data is in VRAM), so the compute cost of decompression becomes a net negative.
\end{itemize}

This confirms that our advanced optimizations (Compression, Hybrid) are specifically targeted at \textbf{Massive, Sparse, Power-Law Graphs} (like Friendster) where memory bandwidth and load imbalance are the primary constraints. For smaller uniform graphs, simpler solvers (V1/V3.1) are superior.
