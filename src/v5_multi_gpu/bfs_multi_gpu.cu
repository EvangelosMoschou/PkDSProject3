...
// =============================================================================
// Simulated Multi-GPU Kernels & Logic
// =============================================================================

#include <omp.h>

__global__ void bfsThreadKernel_MultiGPU(
    ...
    int my_gpu_id, int nodes_per_gpu, node_t num_nodes, int num_gpus) 
{
    ...
    if (tid < q_size) {
        ...
        for (edge_t e = start; e < end; e++) {
            ...
            // Handle edge case where distribution modulo places last nodes outside
            if (target_gpu >= num_gpus) target_gpu = num_gpus - 1;

            if (target_gpu == my_gpu_id) {
                // Local vertex exactly matches owner
                level_t old = atomicCAS(&distances[v], UNVISITED, current_level + 1);
                if (old == UNVISITED) {
                    int idx = atomicAdd(next_frontier_size, 1);
                    next_frontier[idx] = v;
                ...
                // Remote vertex: Push into export queue directed to target_gpu
                // Since this queue is exclusive to (my_gpu_id -> target_gpu), 
                // atomics on out_queue_sizes[target_gpu] will only contend within THIS thread block/grid
...
__global__ void mergeIncomingQueues(
    node_t *incoming_queue, int incoming_size,
    level_t *distances,
    node_t *next_frontier,
    int *next_frontier_size,
    level_t current_level) 
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid < incoming_size) {
        node_t v = incoming_queue[tid];
        
        // This vertex absolutely belongs to our partition, so update distance
        level_t old = atomicCAS(&distances[v], UNVISITED, current_level + 1);
        if (old == UNVISITED) {
            int idx = atomicAdd(next_frontier_size, 1);
            next_frontier[idx] = v;
...
BFSResult *solveBFSMultiGPUSimulated(CSRGraph *graph, node_t source, int num_gpus) {
    node_t num_nodes = graph->num_nodes;
    node_t nodes_per_gpu = (num_nodes + num_gpus - 1) / num_gpus;

    printf("Initializing Multi-GPU Simulation (Local Devices: %d)\n", num_gpus);
    printf("Partition strategy: 1D Vertex Cut (~%d nodes/GPU)\n", nodes_per_gpu);
    ...
    for (int i = 0; i < num_gpus; i++) {
        ...
        for (int j = 0; j < num_gpus; j++) {
            ...
            cudaMallocManaged(&d_msg_queue_sizes[i][j], sizeof(int));
        }
        
        // Copy array of pointers to device
        cudaMalloc(&d_ptr_msg_queues[i], num_gpus * sizeof(node_t*));
        cudaMemcpy(d_ptr_msg_queues[i], msg_queues[i], num_gpus * sizeof(node_t*), cudaMemcpyHostToDevice);
        
        cudaMalloc(&d_ptr_msg_queue_sizes[i], num_gpus * sizeof(int*));
        cudaMemcpy(d_ptr_msg_queue_sizes[i], d_msg_queue_sizes[i], num_gpus * sizeof(int*), cudaMemcpyHostToDevice);
    }
    ...
    while (true) {
        ...
        for (int i = 0; i < num_gpus; i++) total_frontier += frontier_sizes[i];
        
        if (total_frontier == 0) break;
        
        if (level < 5) printf("Level %d: Global Frontier=%d\n", level, total_frontier);

        // ====================================================
        // PHASE 1: Local Computation & Message Generation
        // ====================================================
        #pragma omp parallel num_threads(num_gpus)
        {
            ...
            if (frontier_sizes[gpu_id] > 0) {
                int grid = (frontier_sizes[gpu_id] + 255) / 256;
                bfsThreadKernel_MultiGPU<<<grid, 256, 0, stream>>>(
                    frontiers[gpu_id], frontier_sizes[gpu_id],
                    graph->d_row_ptr, graph->d_col_idx,
                    distances, next_frontiers[gpu_id], d_next_frontier_sizes[gpu_id],
                    level, d_ptr_msg_queues[gpu_id], d_ptr_msg_queue_sizes[gpu_id],
                    gpu_id, nodes_per_gpu, num_nodes, num_gpus
                );
            }
            
            cudaStreamSynchronize(stream);
            cudaStreamDestroy(stream);
        } // implicit OpenMP barrier protects inter-phase sync!

        // ====================================================
        // PHASE 2: Message Exchange & Merge
        // ====================================================
        #pragma omp parallel num_threads(num_gpus)
        {
            int gpu_id = omp_get_thread_num();
            ...
            // Read queues addressed TO ME (receiver = gpu_id) from all OTHER senders (sender = j)
            for (int sender = 0; sender < num_gpus; sender++) {
                if (sender == gpu_id) continue;
                
                // Read via unified memory cache
                int incoming_size = *d_msg_queue_sizes[sender][gpu_id]; 
                
                if (incoming_size > 0) {
                    int grid = (incoming_size + 255) / 256;
                    mergeIncomingQueues<<<grid, 256, 0, stream>>>(
                        msg_queues[sender][gpu_id], incoming_size,
                        distances, next_frontiers[gpu_id], d_next_frontier_sizes[gpu_id],
                        level
                    );
                }
            ...
            // Swap pointers and read sizes
            node_t *temp = frontiers[gpu_id];
            frontiers[gpu_id] = next_frontiers[gpu_id];
            next_frontiers[gpu_id] = temp;
            
            frontier_sizes[gpu_id] = *d_next_frontier_sizes[gpu_id];
    ...
    cudaFree(distances);
    for (int i = 0; i < num_gpus; i++) {
        cudaFree(frontiers[i]);
        cudaFree(next_frontiers[i]);
        cudaFree(d_next_frontier_sizes[i]);
        cudaFree(d_ptr_msg_queues[i]);
        cudaFree(d_ptr_msg_queue_sizes[i]);
        for (int j = 0; j < num_gpus; j++) {
